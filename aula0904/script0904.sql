...
select max(salario) as maior_renda, min(salario) as menor_renda from empregados where sexo = "F";
select max(salario) as maior_renda, min(salario) as menor_renda from empregados where sexo = "M";
...
select char_lenght(nome) as tamanho_nome, (char_lenght(nome) + (rand() * char_length(nome)) * cod_emp) as id_aleatorio from empregado;

select
replace(nome, "A", "") as nome_modificado,
    char_length(nome) as tamanho_nome,
    (char_length(nome) + (rand() * char_length(nome) + (rand() * char_length(nome)) * cod_emp)) as id_aleatorio
from empregado;
